...
const SharedChatbot = (props) => {
  ...
  //whenever selectedchatid or forceupdate changes, call handle load chat
  useEffect(() => {
  if (props.selectedChatId) {
    handleLoadChat();
  }
  ...
  const handleLoadChat = async () => {
    ...
    try {
      const response = await fetcher("retrieve-messages-from-chat", {
        ...
        body: JSON.stringify({
          chat_id: props.selectedChatId,
          chat_type: "shared", //hardcoded the chat type here, maybe this will work?
        }),
      });
      ...
      const response_data = await response.json();
      console.log("Loaded messages:", response_data);
